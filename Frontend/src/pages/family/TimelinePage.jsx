...
import api from "../../utils/axios";
import { useAuth } from "../../utils/authContext";
import StoryCard from "../../components/TimelineComponent/TimelineCard"; // your full card code
...
export default function MemberFamilyPage() {
  ...
  // Fetch Stories for selected family
  const fetchStories = useCallback(async () => {
    if (!familyId) return;
    setLoading(true);
    setError(null);
    ...
    <div className="flex flex-col lg:flex-row bg-gray-50 min-h-screen">
      ...
      <aside className="w-full lg:w-96 bg-white border-l border-gray-200 p-6 shadow-md">
        ...
        <div className="space-y-3 text-sm text-gray-700">
          <p>👑 <span className="font-semibold">Male Root:</span> {maleRoot ? maleRoot.fullname : "N/A"}</p>
          <p>👑 <span className="font-semibold">Female Root:</span> {femaleRoot ? femaleRoot.fullname : "N/A"}</p>
          <p>💍 <span className="font-semibold">Marriage Date:</span> {marriage_date ? new Date(marriage_date).toLocaleDateString() : "N/A"}</p>
          <p>🔐 <span className="font-semibold">Invitation Code:</span> <span className="text-purple-600 font-mono">{invitation_code}</span></p>
        </div>
      </aside>
    </div>
  );
