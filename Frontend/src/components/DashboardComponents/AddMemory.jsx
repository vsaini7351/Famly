...
const AddMemory = ({ familyId }) => {
  ...
  const addTextBlock = () => {
    if (blocks.length > 0 && blocks[blocks.length - 1].type === "text" && blocks[blocks.length - 1].text.trim() === "") {
      alert("⚠️ Fill previous text before adding a new one.");
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white shadow-xl rounded-2xl p-8 mt-6 border border-gray-200">
      <h2 className="text-2xl font-semibold text-center text-gray-800 mb-6">Add New Memory</h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        <input type="text" placeholder="Title *" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full border rounded-lg p-3 text-lg" />
        <input type="text" placeholder="Caption" value={caption} onChange={(e) => setCaption(e.target.value)} className="w-full border rounded-lg p-3 text-lg" />
        <input type="date" value={memoryDate} onChange={(e) => setMemoryDate(e.target.value)} className="w-full border rounded-lg p-3 text-lg" />

        {/* Tags */}
        <input type="text" placeholder="Press Enter to add tags" onKeyDown={handleTagChange} className="w-full border rounded-lg p-3 text-lg mb-2" />
        <div className="flex flex-wrap gap-2 mb-2">{tags.map((tag, i) => <span key={i} onClick={() => removeTag(tag)} className="bg-blue-100 text-blue-700 px-4 py-2 rounded-lg cursor-pointer">#{tag}</span>)}</div>

        {/* Add block buttons */}
        <div className="flex gap-4 mb-4">
          <button type="button" onClick={addTextBlock} className="bg-green-100 text-green-800 px-4 py-2 rounded-lg hover:bg-green-200">➕ Add Text Memory</button>
          <label className="bg-purple-100 text-purple-800 px-4 py-2 rounded-lg hover:bg-purple-200 cursor-pointer">
            📸 Add Media Memory
        ...
        {/* Render blocks */}
        {blocks.map((b, i) => (
          <div key={i} className="border rounded-lg p-3 mb-4 relative">
            <button type="button" onClick={() => removeBlock(i)} className="absolute top-2 right-2 text-red-600 font-bold">✖</button>
            {b.type === "media" && b.file && (
              <>
                {b.file.type.startsWith("image") && <img src={URL.createObjectURL(b.file)} alt="preview" className="w-full h-48 object-cover rounded-lg mb-2" />}
                {b.file.type.startsWith("video") && <video src={URL.createObjectURL(b.file)} controls className="w-full h-48 rounded-lg mb-2 object-cover" />}
                {b.file.type.startsWith("audio") && <audio src={URL.createObjectURL(b.file)} controls className="w-full mb-2" />}
              </>
            )}
            <textarea
              value={b.text}
              onChange={(e) => handleBlockTextChange(i, e.target.value)}
              placeholder={b.type === "media" ? "Add description for media" : "Write your memory text here..."}
              className="w-full border rounded-lg p-3 text-lg"
            />
          </div>
        ))}

        <button type="submit" disabled={uploading} className={`w-full py-3 rounded-lg text-white font-semibold ${uploading ? "bg-gray-400" : "bg-blue-600 hover:bg-blue-700"}`}>
          {uploading ? "Uploading..." : "Upload Memory"}
        </button>
      </form>
    </div>
